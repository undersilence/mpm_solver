...
public:
  ...
  template <class Ty> data::Array<Ty>& row();

  Tag tag() const { return row2id; };
...
private:
  ...
  template <class Ty> data::Array<Ty>& _typed_row();

  template <class Ty> data::Array<Ty>& _typed_row(size_t row);
...
public:
  template <class... Tys> Archetype& add_to_archetype(Archetype& src);

  template <class... Tys> Archetype& del_from_archetype(Archetype& src);
...
template <class... Tys> inline Tag World::tag(Tag exist_tag) {
  auto new_tag = Tag{get_id<Tys>()...};
...
template <class... Tys> Archetype& World::add_to_archetype(Archetype& src_archetype) {
  auto&& new_tag = tag<Tys...>(src_archetype.tag());
  auto& dst_archetype = archetype(std::move(new_tag));

  if (new_tag.size() > dst_archetype.rows()) {
    dst_archetype.mimic_rows(src_archetype);
    dst_archetype.template add_rows<Tys...>();
  }

  return dst_archetype;
}

template <class... Tys> Archetype& World::del_from_archetype(Archetype& src_archetype) {
  ...
  if (cur_tag.size() > dst_archetype.rows()) {
    dst_archetype.mimic_rows(src_archetype);
    dst_archetype.template del_rows<Tys...>();
  }
...
template <class Ty> data::Array<Ty>& Table::row() { return _typed_row<Ty>(); }

inline bool Table::has(Id id) { return id2col.contains(id); }

template <class... Ty> bool Table::has() {
...
template <class... Ty> void Table::set(Id id, Ty&&... val) {
  ...
  (([&](auto&& val){
    ...
    if(data_row.size() > col) {
      ...
      // Here the append is needed to add this component, because this entity is partially initialized by outside moving.
...
template <class Ty> data::Array<Ty>& Table::_typed_row() {
  return _typed_row<Ty>(id2row.at(World::get_id<Ty>()));
}

template <class Ty> data::Array<Ty>& Table::_typed_row(size_t row) {
